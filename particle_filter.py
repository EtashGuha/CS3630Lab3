...
from utils import grid_distance, rotate_point, diff_heading_deg, add_odometry_noise, add_gaussian_noise, add_marker_measurement_noise
import setting
import math
import numpy as np
import copy
...
def motion_update(particles, odom, grid):
    ...
    for particle in particles:
        ...
        particle.x += add_gaussian_noise(x, setting.ODOM_TRANS_SIGMA)
        particle.y += add_gaussian_noise(y, setting.ODOM_TRANS_SIGMA)
        particle.h += add_gaussian_noise(dh, setting.ODOM_HEAD_SIGMA)
        ...
        if measuredMarkerListLength:
            ...
            if not markersLength: weights.append(0)
                ...
                for marker, worst in lst: confidence *= math.exp(-1 * (math.pow(grid_distance(marker[0], 
                    marker[1], worst[0], worst[1]), 2) / (2 * math.pow(setting.MARKER_TRANS_SIGMA, 2)) 
                + math.pow(diff_heading_deg(marker[2], worst[2]), 2) / (2 * math.pow(setting.MARKER_ROT_SIGMA, 2))))
